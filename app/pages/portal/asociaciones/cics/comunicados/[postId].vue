<template>
  <main class="">
    ...
    <main v-if="data && status === 'success'">
      ...
      <p class="text-sm text-muted-foreground mb-4" v-html="formatPublishedDate(data.data.publishedAt)" />
...
useCustomPageTitle(data.value?.data?.title ? `Comunicado - ${data.value?.data?.title}` : 'Comunicado')
definePageMeta({
  title: 'Comunicado'
})
</script>
