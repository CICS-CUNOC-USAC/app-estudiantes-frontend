<template>
    <div class="flex">
        <div class="h-[93vh] bg-cics-silver-pale dark:bg-neutral-900">
            <DisplayModeSelector :model-value="scheduleType" :classrooms="classrooms" :hours="availableHours || []"
                @update:selected-periods="(val: Hour[]) => {
            ...
            <div class="grid grid-cols-1">
                <div class="mb-4 grid grid-cols-6">
                    <div class="col-start-1">
                        <ArrowedCombobox :options="[
                            'Todas las Carreras',
                            'Area Comun',
                            ...availableCareers.map((value: Career) => value.name)
                        ]" :default-value="'Todas las Carreras'" @update:selected-option="(val: string) => {
                    selectedCareer = val
                }
                "></ArrowedCombobox>
                    </div>
                    <div class="col-span-2 col-start-3">
                        <ToggleGroupRoot :model-value="coursesMode" @update:model-value="(val) => {
                                if (val) coursesMode = val as 'lectures' | 'laboratories'
                            }
                            " class="flex justify-center gap-1">
                            <ToggleGroupItem value="lectures"
                    ...
                    <div class="col-span-2 col-start-5">
                        <CInputText v-model="search" placeholder="Buscar un curso" prepend-icon="lucide:search"
                ...
                <ScrollAreaRoot ref="scrollerSchedules">
                    <ScrollAreaViewport class="max-h-[80vh]">
                        <ClassScheduleV1 v-if="classrooms && schedules && hours" :key="schedulesKey" :hours="hours"
